...
class User extends CI_Controller {
	...
	public function index()
	{
		...
		$this->template->load('template' , 'User/user_data', $data); //ini untuk  view dan menambhkan variabel
	}
	public function add()
	{
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() == FALSE) {
            ...
            {
            	$post = $this->input->post(null, TRUE);
				$this->user_m->edit($post); //<<< INGAT DI GANTI SESUAIKAN
				if($this->db->affected_rows() > 0 ) {
	...
	function username_check() {
		...
		if($query->num_rows() > 0) {
			$this->form_validation->set_message('username_check', '%s Minimal 5 karakter');
		return FALSE;
		}else {
			return TRUE;
		}
